...
<hr />
<b><h1>Search Result related to: <?php $passid = trim($_GET['term']); echo $passid?></h1></b>
...
$selectpost = "SELECT * FROM posts WHERE post_title LIKE '%$passid%' ORDER BY date_published DESC LIMIT $start_from, 10";
...
if ($result->num_rows > 0) {
	...
	$countdata = "SELECT COUNT(post_content) FROM posts WHERE post_title LIKE '%$passid%'";
	...
	echo "<a href='search_post_scan.php?page=".$i."&term=$passid'>".$i."</a> "; 
