...
<div class="row">
	...
	<div class="col-md-7">
		...
		if ($result1->num_rows > 0) {
			...
			if($passid > 1) {
				echo "<a href='tag_scan.php?page=".$backpage."&term=$pterm#$backpage' id='".$backpage."'>Previous Page</a> ";
			}
			
			for ($i=1; $i<=$total_pages; $i++) {
				echo "<a href='tag_scan.php?page=".$i."&term=$pterm#$i' id='".$i."'>".$i."</a> ";
				$num[] = $i;
				$maxval = count($num);
			};
			
			if($passid > 0 && $passid < $maxval) {
			echo "<a href='tag_scan.php?page=".$nextpage."&term=$pterm#$nextpage' id='".$nextpage."'>Next Page</a>";
			}
					...
					$drp = $conn->query($recenntpost);
						...
						if ($drp->num_rows > 0) {
							while($rp = $drp->fetch_assoc()) {
								...
								echo "<p><a href='view_post.php?id=$postid&title=$posttitle' class='rplink' style='text-decoration:none; color: #005c5c; font-size:20px;'><span class='icon'><i class='fa fa-bullhorn'></i>&nbsp;<b>$posttitle</a></b></span></p>";
								echo "<p style='font-size:12px;'>Published on: $pubdate</p>";
							}
						}
					?>
