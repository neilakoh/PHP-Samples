...
<div class="row">
	...
	<div class="col-md-7">
		...
		$getpost = "SELECT * FROM posts ORDER BY date_published DESC LIMIT $start_from, 5";
		...
		if ($result1->num_rows > 0) {
			while($row = $result1->fetch_assoc()) {
				...
				if (strlen($string) > 300) {
					...
					$string = substr($stringCut, 0, strrpos($stringCut, ' '))."... <a href='view_post_active.php?id=$postid&title=$posttitle' class='tinylinks' style='text-decoration: none; padding-top: 2px; padding-bottom: 2px; padding-left: 5px; padding-right: 5px; background: #005c5c; color:#ffffff; border-radius: 2px;'>Read More</a>"; 
				...
				<div class="container" id="blogpost" style="background-color:#E0F1F8; width: 100%; height: auto; padding: 10px; box-shadow: 0 1px 2px 0 #bcbfc0; margin-bottom: 15px;">
					<div class='blogthumbs' style='height: 100%; width:120px; float: left; margin-right: 10px; margin-bottom: 20px;'><img src='images/postthumbnails/<?php echo $thumbnail;?>' height='120px' width='120px' style='float:left;'></div>
			...
			if($passid > 1) {
				echo "<a href='main.php?page=".$backpage."#$backpage' id='".$backpage."'>Previous Page</a> ";
			}
			
			for ($i=1; $i<=$total_pages; $i++) {
				echo "<a href='main.php?page=".$i."#$i' id='".$i."'>".$i."</a> ";
				$num[] = $i;
				$maxval = count($num);
			};
			
			if($passid > 0 && $passid < $maxval) {
			echo "<a href='main.php?page=".$nextpage."#$nextpage' id='".$nextpage."'>Next Page</a>";
			}
