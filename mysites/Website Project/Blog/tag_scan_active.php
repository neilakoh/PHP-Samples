...
<div class="row">
	...
	<div class="col-md-7">
		...
		$getpost = "SELECT * FROM posts WHERE post_tags LIKE '%$pterm%' ORDER BY date_published DESC LIMIT $start_from, 5";
		$result1 = $conn->query($getpost);

		$countdata = "SELECT COUNT(*) FROM posts WHERE post_tags LIKE '%$pterm%'";
		...
		if ($result1->num_rows > 0) {
			while($row = $result1->fetch_assoc()) {
				...
				if (strlen($string) > 300) {
					...
					$string = substr($stringCut, 0, strrpos($stringCut, ' '))."... <a href='view_post_active.php?id=$postid&title=$posttitle' class='tinylinks' style='text-decoration: none; padding-top: 2px; padding-bottom: 2px; padding-left: 5px; padding-right: 5px; background: #005c5c; color:#ffffff; border-radius: 5px;'>Read More</a>"; 
				...
				<div class="container" id="blogpost" style="background-color:#E0F1F8; width: 100%; height: auto; padding: 10px; box-shadow: 0 1px 2px 0 #bcbfc0; margin-bottom: 15px;">
					...
					<div class="blogtitle" style="position: relative; top: -25px; font-family: 'Lobster', cursive;"><a href="view_post_active.php?id=<?php echo $postid; ?>&title=<?php echo $posttitle; ?>" class="posttitlelink" style="text-decoration:none; color:#005c5c;"><h2><?php echo $posttitle; ?></h2></a></div>
					...
					echo "<a href='tag_display_active.php?id=$elem' class='tinylinks' style='text-decoration: none; padding-top: 2px; padding-bottom: 2px; padding-left: 5px; padding-right: 5px; background: #005c5c; color:#ffffff; border-radius: 5px;'>$elem</a>&nbsp;";
					}
					?>
					</div>
				</div>
			...
			if($passid > 1) {
				echo "<a href='tag_scan_active.php?page=".$backpage."&term=$pterm#$backpage' id='".$backpage."'>Previous Page</a> ";
			}
			
			for ($i=1; $i<=$total_pages; $i++) {
				echo "<a href='tag_scan_active.php?page=".$i."&term=$pterm#$i' id='".$i."'>".$i."</a> ";
				$num[] = $i;
				$maxval = count($num);
			};
			
			if($passid > 0 && $passid < $maxval) {
			echo "<a href='tag_scan_active.php?page=".$nextpage."&term=$pterm#$nextpage' id='".$nextpage."'>Next Page</a>";
			}
