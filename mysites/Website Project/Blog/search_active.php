...
$searchresult = trim($_POST['search']);
?>
...
<div class="row">
	...
	<div class="col-md-7">
		<div class="searchResult"><b>Search Result Related to:</b> <?php echo $searchresult;?></div>
		<?php
		include('setup/conn.php');
		if (isset($_GET["page"])) {
			$page  = $_GET["page"]; 
		...
		$getpost = "SELECT * FROM posts WHERE post_title LIKE '%$searchresult%' ORDER BY date_published DESC LIMIT $start_from, 5";
		$result1 = $conn->query($getpost);

		$countdata = "SELECT COUNT(*) FROM posts WHERE post_title LIKE '%$searchresult%'";
		...
		if ($result1->num_rows > 0) {
			...
			for ($i=1; $i<=$total_pages; $i++) {
				echo "<a href='search_scan_active.php?page=".$i."&term=$searchresult#$i' id='".$i."'>".$i."</a> ";
			};
